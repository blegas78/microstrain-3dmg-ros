...
systemctl disable microstrain


# Here is where we remove scripts, services, etc.
echo " - Removing scripts..."
cd
if [ "x"`systemctl list-units | grep -c microstrain.service` = "x1" ]; then
    echo "Uninstalling microstrain.service"

    source /home/$LIBPANDA_USER/catkin_ws/devel/setup.bash
    rosrun robot_upstart uninstall microstrain
fi
